...
**1. 上传CSV文件：** 准备包含instruction（问题）、output（回答）、chunk（参考内容）三列的CSV文件，点击"上传CSV文件"按钮选择文件，系统会默认自动加载前50条数据。

**2. 查看样本列表：** 左侧显示所有加载的样本，⭕表示待处理，✅表示已校正，❌表示已丢弃。当前正在处理的样本会显示在列表顶部。您可以通过点击"上一条"、"下一条"按钮来切换样本，也可以直接点击样本导航列表中的任意样本实现自由切换。
...
**6. 提交或丢弃：** 确认无误后点击"提交最终样本"保存校正结果，或点击"丢弃此样本"跳过当前样本。已丢弃的样本可以通过点击导航列表重新加载，点击"恢复此样本"按钮可以恢复该样本，但该样本以前的修改会丢失。

**7. 导出数据：** 完成所有样本校正后，点击"导出已校正数据"按钮生成JSON文件，默认文件名格式为"原文件名_时间戳_校正样本数.json"。

**8. 下载文件：** 导出成功后，在"导出文件下载"框中点击下载生成的文件。

**9. 回溯功能（继续之前的校正工作）：** 
   - 点击"回溯已校正数据"按钮，会显示"上传已校正数据JSON文件"上传框。
   - 选择之前导出的JSON文件上传。
   - **重要提醒：务必确保上传的JSON文件是从当前正在处理的原始CSV文件校正导出的！** 系统会检查文件名匹配，如果不匹配会弹出警告。
   - 系统会自动加载JSON文件中的已校正数据，并跳转到第一个未处理的样本，让您可以从中断的地方继续校正工作。

**10. 自动预加载：** 当您正在处理当前批次的最后1个样本时，系统会自动加载下一批样本，无需手动操作。

**注意事项：**
- 所有样本编号从1开始计数。
- 样本导航列表会始终将当前处理的样本显示在顶部，方便查看。
- 使用回溯功能时，请确保JSON文件与CSV文件对应，避免数据混乱。
                """)
...
def create_upload_export_row(components: Dict[str, Any]) -> None:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            # 回溯JSON文件上传框（默认隐藏）
            components['backtrack_upload'] = gr.File(
                label="📂 上传已校正数据JSON文件",
                file_types=[".json"],
                type="filepath",
                height=100,
        ...
        with gr.Column(scale=2):
            ...
            components['backtrack_btn'] = gr.Button(
                "⏮️ 回溯已校正数据",
                size="lg",
...
def create_center_column(components: Dict[str, Any]) -> None:
    ...
    with gr.Group(visible=False) as phase2_group:
        ...
        # 三个按钮并排（带间距）
        with gr.Row(elem_classes=["button-row-with-gap"]):
